...
import { useRef, useState, ReactEventHandler } from "react"
...
export default function CustomOrder() {
  ...
  const updateQuantity = (itemId: string, q: number): void => {
    if (q > 0) {
              ...
              <div className="grid sm:grid-cols-2 gap-4 max-h-96 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent">
                {menuItems.map((item, index) => (
                  <motion.div
                    ...
                    className={`p-6 rounded-2xl border-2 cursor-pointer transition-all duration-300 hover:shadow-lg group ${
            ...
            <div className="lg:sticky lg:top-24">
              ...
              <div className="bg-card border border-border rounded-3xl p-6 shadow-xl">
                ...
                <motion.button
                  ...
                  className={`w-full py-4 rounded-xl font-semibold text-lg flex items-center justify-center gap-3 transition-all duration-300 shadow-lg ${
                    isButtonEnabled
